...
@app.route('/api/diet-chart/update', methods=['POST'])
def update_diet_chart():
    ...
    if not chart_data:
        return jsonify({'success': False, 'message': 'Missing chart data'})
    
    try:
        # Get the most recent active diet chart for the user
        diet_chart = DietChart.query.filter_by(user_email=user_email, is_active=True).order_by(DietChart.created_date.desc()).first()
        
        if diet_chart:
            # Update existing chart
            diet_chart.chart_data = json.dumps(chart_data)
            if user_data:
                diet_chart.user_data = json.dumps(user_data)
            db.session.commit()
            return jsonify({'success': True, 'message': 'Diet chart updated successfully'})
        else:
            # Create new chart if none exists
            new_diet_chart = DietChart(
                user_email=user_email,
                chart_name='Updated Diet Chart',
                chart_data=json.dumps(chart_data),
                user_data=json.dumps(user_data) if user_data else json.dumps({}),
                goal=user_data.get('goal', 'General fitness') if user_data else 'General fitness',
                target_calories=chart_data.get('totalCalories', 2000) if isinstance(chart_data, dict) else 2000
            )
            db.session.add(new_diet_chart)
            db.session.commit()
            return jsonify({'success': True, 'message': 'Diet chart created successfully'})
    except Exception as e:
        db.session.rollback()
        return jsonify({'success': False, 'message': f'Failed to update diet chart: {str(e)}'})
